...
from utils.innvestigate.analyzer import BoundedDeepTaylor, GuidedBackprop
from utils.innvestigate.analyzer import LRPEpsilon, LRPSequentialPresetAFlat, LRPSequentialPresetBFlat
from utils.innvestigate.analyzer import Deconvnet, IntegratedGradients, SmoothGrad
...
class _LRPSubtraction(object):
    ...
    def analyze(self, inputs):
        ...
        if self.scaling:
            analysis = analysis_target * analysis_target.sum(axis=(1, 2), keepdims=True) - analysis_others * analysis_others.sum(axis=(1, 2), keepdims=True)
        else:
            analysis = analysis_target - analysis_others
        #print("result")
...
class GuidedGradCAM(object):
    ...
    def analyze(self, inputs):
        return self.gradcam.analyze(inputs) * self.guidedbackprop.analyze(inputs)[:,:,:,np.newaxis]
