...
def pre_softmax_tensors(Xs, should_find_softmax=True):
    ...
    for x in Xs:
        ...
        if layer.name == "dense_last" and kchecks.contains_activation(layer, activation="softmax"):
            softmax_found = True
...
def pre_sigmoid_tensors(Xs, should_find_sigmoid=True):
    """Finds the tensors that were preceeding a potential softmax."""
    sigmoid_found = False
    ...
    for x in Xs:
        layer, node_index, tensor_index = x._keras_history
        if layer.name == "dense_last" and kchecks.contains_activation(layer, activation="sigmoid"):
            sigmoid_found = True
            if isinstance(layer, keras.layers.Activation):
                ret.append(layer.get_input_at(node_index))
            else:
    ...
    if should_find_sigmoid and not sigmoid_found:
        raise Exception("No sigmoid found.")
