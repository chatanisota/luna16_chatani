...
def spatial_attention(input_feature, name="sp0"):
	...
	max_pool = Lambda(lambda x: K.max(x, axis=3, keepdims=True), name=name+"_dense2")(cbam_feature)
...
def resnet_block(block_input, num_filters, name="res0"):  # Single ResNet block
    ...
    cbam = cbam_block(norm2, 4, name=name)

    sum = Add(name=name+"_add")([block_input, cbam])
    relu2 = Activation('relu', name=name+"_activation2")(sum)

    return relu2


def cbamcnnresnet2d(input):
    '''
